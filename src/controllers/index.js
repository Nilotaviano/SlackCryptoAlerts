...
router.post("/delete", function(req, res) {
  ...
  if (entity == 'alert' || entity == 'alerts') {
    ...
    var query = alerts.chain().where(function(alert) {
      return alert.user == username
    });

    if (currency != 'all') {
      query = query.where(function(alert) {
        return alert.currency == currency
      });

      if (alertPrice != null)
        query = query.where(function(alert) {
          return alert.price == alertPrice
        });
    }
    ...
    var query = calls.chain().where(function(alert) {
      return alert.currency == currency
    });
  ...
  calls.push(function(callback) {
    bittrex.getticker({
      ...
      if (ticker.success) {
        var price = {
          exchange: 'Bittrex',
          bid: ticker.result.Bid.toFixed(8),
          ask: ticker.result.Ask.toFixed(8),
          last: ticker.result.Last.toFixed(8)
        };
        callback(null, price);
      }
      else {
      ...
      if (!err) {
        for (var key in ticker) {
          if (ticker.hasOwnProperty(key) && key == "BTC_" + currency) {
            var price = {
              exchange: 'Poloniex',
              bid: ticker[key].highestBid,
              ask: ticker[key].lowestAsk,
